...
!     Read input parameters
      ...
      READ (1, *) 
      READ (1, *) YKAPPA, RSHIFT, DPMU, FIELD, ANGL 
      READ (1, *)
...
!     ============================================================================================== 
      ...
      DO J = 1, NPART
         ...
         IF (INDXP < 1 .OR. INDXP > NPART) THEN
            WRITE (MYUNIT,'(A,I5)') 'Error in index  selected particle', INDXP
            STOP
         ENDIF
...
!     ============================================================================================== 
      ...
      INTEGER          :: J1, J2, J7, J8, I, J, J3, J4, J5, J6, OFFSET
      DOUBLE PRECISION :: RIJ(3), RIJSQ, ABSRIJ, R2, R4, EXPFCT, RSS(3), RSSSQ, PERS
...
!     Yukawa contribution
      ...
      CALL SNENRG_DIPOLE_REALSPACE(J2,PERS)
...
!     ============================================================================================== 
      ...
      SUBROUTINE SNENRG_DIPOLE_REALSPACE(J2,PERS)

      USE COMMONS, ONLY: ALPHA, ALPSQ, BOXL, DPMUSQ, INVRPI, NPART, E, RS, NRBSITE

      IMPLICIT NONE

      INTEGER          :: I, J, J2, J3, J4, J5, J6, J7, J8, OFFSET, J1
         ...
         DO I = NRBSITE/2+1, NRBSITE
            ...
            DO J = NRBSITE/2+1, NRBSITE
               ...
               IF (RSQ < RCUTSQ) THEN
                 ...
                 DOTALP  = DOT_PRODUCT(RIJ(:),EI(:))
                 DOTBET  = DOT_PRODUCT(RIJ(:),EJ(:))
...
!     ==============================================================================================
      ...
      SUBROUTINE SNENRG_DIPOLE_FOURIERSPACE(J2,DPEKS,MC,MS,SUMC,SUMS)

      USE COMMONS, ONLY: NRBSITE, ALPHA, BOXL, GU, NC, NCSQMAX, NPART, MAXK, PI, RS, E, SUMCO, SUMSO, MCO, MSO, KFCTR
...
!     ==============================================================================================
      ...
      INTEGER          :: J1, J2, J3, J4, J5, J6, J7, J8, I, J, OFFSET
      ...
      DO J1 = 1, NPART
         ...
         DO J2 = 1, NRBSITE
            ...
            J4 = NRBSITE*(J1-1) + J2 
            RS(J4,:) = RI(:) + MATMUL(RM(:,:),RBSITE(J2,:))
            IF (J2 > NRBSITE/2) THEN
               E(J4,:)   = MATMUL(RM(:,:),RBUV(J2,:))
            ENDIF
         ENDDO
      ENDDO
...
!     ==============================================================================================
      ...
      INTEGER          :: J1, J2, J3, J4, J5, J6, J7, J8, I, J
      DOUBLE PRECISION :: RIJ(3), EI(3), EJ(3)
      DOUBLE PRECISION :: ABSR, RSQ, R2, RCUTSQ, DOTALP, DOTBET, DOTGAM
      ...
      PER = PER - 2.D0*DPMUSQ*ALPHA**3*NPART*INVRPI/3.D0
...
!     ==============================================================================================
      ...
      USE COMMONS, ONLY: ALPHA, BOXL, GU, NC, NCSQMAX, NPART, MAXK, PI, E, SUMCO, SUMSO, MCO, MSO, KFCTR, NRBSITE

      IMPLICIT NONE

      INTEGER          :: NVV, VN(3), NX, NY, NZ, J, TOTK, J1, J8, J2, J3
      DOUBLE PRECISION :: DUMMY, PC, PS, T, W
      DOUBLE PRECISION :: VC(3), VS(3), TCOS((NPART*(NRBSITE/2)),0:NC,3), TSIN((NPART*(NRBSITE/2)),0:NC,3)
      DOUBLE PRECISION :: PEK, FK(NPART,3), GK(NPART,3)
            ...
            DO NX =-NC, NC
               ...
               DO J = 1, NPART
                  DO J1 = NRBSITE/2 + 1, NRBSITE
                    ...
                    PS = VS(1)*VC(2)*VC(3) + VC(1)*VS(2)*VC(3) + VC(1)*VC(2)*VS(3) - VS(1)*VS(2)*VS(3)
                    DUMMY = NX*E(J8,1) + NY*E(J8,2) + NZ*E(J8,3)
                    MCO(TOTK,J3) = PC*DUMMY
                    MSO(TOTK,J3) = PS*DUMMY
                    SUMCO(TOTK)  = SUMCO(TOTK) + MCO(TOTK,J3)
                    SUMSO(TOTK)  = SUMSO(TOTK) + MSO(TOTK,J3)
                  ENDDO
               ENDDO
...
!     ==============================================================================================
      ...
      USE COMMONS, ONLY: BOXL, NC, NPART, RS, NRBSITE

      IMPLICIT NONE

      INTEGER          :: J, K, J8, J2, J3, J1
      DOUBLE PRECISION :: T(3), TT(3), U(3), W(3), TCOS((NPART*(NRBSITE/2)),0:NC,3), TSIN((NPART*(NRBSITE/2)),0:NC,3)
      DOUBLE PRECISION, PARAMETER :: TWOPI = 8.D0*DATAN(1.D0)
...
!     ==============================================================================================
      ...
      SUBROUTINE DEFMULTSHIFTDP()

      USE COMMONS, ONLY: NRBSITE, RBSITE, RBUV
...
!     ==============================================================================================
      ...
      X = NRBSITE/2
      WRITE(31,'(I6)') NPART*(NRBSITE)
      WRITE(31, *)
      DO J1 = 1, NPART
         R(J1,:) = R(J1,:) - ANINT(R(J1,:)/BOXL)*BOXL
         QI(:) = Q(J1,:)
         CALL ROTMAT(QI, RM)
         !WRITE(31,'(A5,1X,3F20.10)') 'O ', R(J1,1), R(J1,2), R(J1,3)
         DO J2 = 1, X
            SR(J2,:) = R(J1,:) + MATMUL(RM,RBSITE(J2,:))
            WRITE(31,'(A5,1X,3F20.10)') 'O ', SR(J2,1), SR(J2,2), SR(J2,3)
         ENDDO
         DO J3 = X+1, NRBSITE
            SR(J3,:) = R(J1,:) + MATMUL(RM,RBSITE(J3,:))
            UV(:)     = MATMUL(RM(:,:),RBUV(J3,:))
            WRITE(31,'(A4,3F20.10,2X,A12,2X,3F20.10)') 'C', SR(J3,1), SR(J3,2), SR(J3,3), 'atom_vector', UV(1), UV(2), UV(3)
         ENDDO
      ENDDO
...
!      DOUBLE PRECISION :: QI(4), RM(3,3), RBCOORDS(6)

!      OPEN(UNIT = 31, FILE = 'multfinalconfig.xyz', STATUS = 'UNKNOWN')
...
!               RBCOORDS(1:3) = QMINP(J1,J5-2:J5) + MATMUL(RM(:,:),RBSITE(J2,:))
!               IF (J2 <= NRBSITES/2) THEN
!                  WRITE(VIEWUNIT,'(A4,3F20.10)') 'O', RBCOORDS(1), RBCOORDS(2), RBCOORDS(3)
!               ELSE
!                  RBCOORDS(4:6) = MATMUL(RM(:,:),RBUV(J2,:))
!                  WRITE(VIEWUNIT,'(A4,3F20.10,2X,A12,2X,3F20.10)') 'C', RBCOORDS(1), RBCOORDS(2), RBCOORDS(3), &
!                 'atom_vector', RBCOORDS(4), RBCOORDS(5), RBCOORDS(6)
!               ENDIF
...
!      END SUBROUTINE VIEWMULTSHIFTDP
!     ==============================================================================================
...
!     ==============================================================================================
      ...
      INTEGER          :: G, I, K, NUC, IX, IY, IZ, M, IREF, J1, J2
      DOUBLE PRECISION :: BOXLH, UCL, UCLH, RROOT3
      DOUBLE PRECISION :: RANF, DUMMY, QN(4), RM(3,3)
